...
outline, RMS), never from the original audio.
    ...
    python -m games.sm64.voicefx <takes dir> [--report]
"""
import json
import os
import sys
...
CHAR = {
    "mario": {"formant_st": 2.0, "expressive": 1.25, "max_speed": 2.0, "max_st": 12.0, "default_f0": 420.0},
    "peach": {"formant_st": 3.0, "expressive": 1.15, "max_speed": 1.8, "max_st": 12.0, "default_f0": 260.0},
}
...
def process(x, sr, slot, text, prof, words, text_who="mario"):
    ...
    cands = cands or [x]
    if len(cands) > 1:
        from .voices import _norm, _hear
        def score(c):
            dur = len(c) / sr
    ...
    info = {}
    x = close_gaps(x, sr)
    who = "peach" if "peach" in text_who else "mario"
    ch = CHAR[who]
...
WORD_LINES = {"here_we_go", "game_over", "hello", "press_start", "so_longa", "ima_tired", "lets_a_go", "its_a_me",
              "mama_mia", "okey_dokey", "thank_you", "peach_"}
...
def main(argv):
    ...
    for name, (x, sr, prof) in raw.items():
        if name not in L or len(x) < sr * 0.05:
            continue
        p, v = L[name]
        words = any(k in name for k in WORD_LINES)
