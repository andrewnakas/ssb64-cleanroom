...
Results are cached in games/sm64/voices/<slot>.wav, so builds on machines
without the speech engine reuse them.

    python -m games.sm64.voices build        # (re)speak every line, print fit summary
    python -m games.sm64.voices check        # speech-recognition intelligibility report
"""
import json
import os
import struct
...
VOICE = {"mario": ("Microsoft David Desktop", "+15%"), "peach": ("Microsoft Zira Desktop", "+5%")}
...
def _speak(jobs):
    ...
    subprocess.run(["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File",
                    os.path.join(HERE, "tts.ps1"), jf], check=True, capture_output=True)
...
CHARACTERS = {
    "mario": {"model": "en_US-ryan-high", "semitones": 3.0, "length": 0.95, "noise": 0.85, "noise_w": 1.0},
    "peach_quick": {"model": "en_US-amy-medium", "semitones": 2.0, "length": 0.85, "noise": 0.7, "noise_w": 0.9},
    "peach": {"model": "en_US-kristin-medium", "semitones": 1.5, "length": 0.9, "noise": 0.7, "noise_w": 0.9},
}
_VOICES = {}
...
def check():
    ...
    out = subprocess.run(["powershell", "-NoProfile", "-ExecutionPolicy", "Bypass", "-File",
                          os.path.join(HERE, "asr_check.ps1"), adir], capture_output=True, text=True).stdout
